...
d_robot2 <- d_clean %>%
  ...
  mutate(race_white_cat = 
           factor(ifelse(is.na(race_white), "non-white", 
                         ifelse(race_white == 1, "white", NA))),
         religion_christian_cat = 
           factor(ifelse(is.na(religion_christianity), "non-christian",
                         ifelse(religion_christianity == 1, "christian", NA))),
         age_cat =
           factor(ifelse(age_approx < 30, "young",
                         ifelse(age_approx >= 30, "old",
                                NA))))
...
g <- ggbiplot(pca_robot2, obs.scale = 1, var.scale = 1, 
              ...
              # groups = d_robot2$religion_christian_cat,
...
g <- ggbiplot(pca_beetle2, obs.scale = 1, var.scale = 1, 
              ...
              # groups = d_beetle2$religion_christian_cat,
