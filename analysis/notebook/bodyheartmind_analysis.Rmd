...
```{r data cleanup, include = F, warnings = F, echo = F}
# clean up datasets
d1 <- cleanup("study 1")
d2 <- cleanup("study 2")
d3 <- cleanup("study 3")
...
```{r s1 how many factors, include = F}
...
```{r s2 how many factors, include = F}
# examine eigenvalues and variance explained
...
```{r s3 how many factors, include = F}
# examine eigenvalues and variance explained
...
```{r s4 how many factors, include = F}
# examine eigenvalues and variance explained
...
order_s1 <- loadings(fa.sort(efa_d1_all_rotatedN))[] %>%
  ...
  rename(s1_MR1 = MR1, s1_MR2 = MR2, s1_MR3 = MR3)

order_s2 <- loadings(fa.sort(efa_d2_all_rotatedN))[] %>%
  data.frame() %>%
  rownames_to_column(var = "mc") %>%
  rename(s2_MR1 = MR1, s2_MR2 = MR2, s2_MR3 = MR3)

order_s3 <- loadings(fa.sort(efa_d3_all_rotatedN))[] %>%
  data.frame() %>%
  rownames_to_column(var = "mc") %>%
  rename(s3_MR1 = MR3, s3_MR2 = MR1, s3_MR3 = MR2) # note discrepancy

order_s4 <- loadings(fa.sort(efa_d4_all_rotatedN))[] %>%
  data.frame() %>%
  rownames_to_column(var = "mc") %>%
  rename(s4_MR1 = MR1, s4_MR2 = MR2, s4_MR3 = MR3)
...
```{r figure S1, fig.width = 10, fig.height = 6}
...
# plot!
figS1 <- plot_ly(data = scatter_plotting, 
...
```{r figure S2, fig.width = 6, fig.height = 12}
# make dataframe
...
s123 <- ggplot(s123_plotting, 
               aes(y = Mean, x = reorder(wording, desc(s1_order)), colour = s1_color,
                   shape = study)) +
  geom_point(stat = "identity", position = position_dodge(width = 0.8), size = 4) +
...
```{r figure S3, fig.width = 12, fig.height = 6}
# make dataframe
