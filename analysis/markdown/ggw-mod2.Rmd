...
# build model from EFA factor structure
buildModel <- function (chosenStructure, chosenFactor) {
  factorName <- paste0("mc_", chosenFactor)
  factorLoading <- paste0("loading_", chosenFactor)
  mcs <- chosenStructure
  pos <- mcs[, factorName][is.na(mcs[factorName]) == F & mcs[factorLoading] > 0]
  neg <- mcs[, factorName][is.na(mcs[factorName]) == F & mcs[factorLoading] < 0]
  factorModel <- paste0(chosenFactor, " =~ ", paste(pos, collapse = " + "))
  if(length(neg) > 0) {
    factorModel <- paste0(factorModel, " + (-x)* ", paste(neg, collapse = " - "))
  }
  return(factorModel)
}
...
modelDD_MR1 <- buildModel(d1_fStructure, "MR1")
modelDD_MR2 <- buildModel(d1_fStructure, "MR2")
modelDD_MR3 <- buildModel(d1_fStructure, "MR3")
modelDD_allFactors <- paste(modelDD_MR1, "\n", 
                            modelDD_MR2, "\n", modelDD_MR3, collapse = "")
modelDD <- gsub(" \n ", "\n", as.character(noquote(modelDD_allFactors)))
