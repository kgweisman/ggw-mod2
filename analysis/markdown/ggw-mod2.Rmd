...
howManyFactors <- function (df) {
  ...
  countEigen <- function(df) {
    ...
    nLarge <- length(eigenVals[eigenVals > 1])
    return(nLarge)
  }
  
  # scree plot: look for elbow
  scree_df <- scree(df, main = "Scree plot: Study 1, BEETLE condition")
  
  # eigenvalues: count how many > 1
  eigen_df <- countEigen(df)
  text(x = 20, y = 10, 
       labels = paste(eigen_df, "eigenvalues > 1 (for EFA)"))
...
getFactorLoadings <- function(efa) {
  ...
  for(i in 1:efa$factors) {
    ...
    df <- data.frame(X = sort(efa$loadings[,1], decreasing = T))
    names(df) <- factorNum
...
print(efa_d1_beetle_unrotatedN_loadings)
```
