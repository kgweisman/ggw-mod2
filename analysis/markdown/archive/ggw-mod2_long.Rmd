...
howManyFactors <- function (df) {
  ...
  #                              rotate = "oblimin", cor = chosenCorType, plot = FALSE)
}
...
extractFStructure <- function(efa) {
  ...
  d_structure <- data.frame(loadings(efa)[]) %>%
    ...
    mutate(
      bestFactor =
        ifelse(abs(MR1) == abs(MR2) & MR1 == abs(MR3),
               "all tied",
               ifelse(abs(MR1) == abs(MR2),
                      "MR1 and MR2 tied",
                      ifelse(abs(MR1) == abs(MR3),
                             "MR1 and MR3 tied",
                             ifelse(abs(MR2) == abs(MR3),
                                    "MR2 and MR3 tied",
                                    ifelse(abs(MR1) > abs(MR2),
                                           ifelse(abs(MR1) > abs(MR3),
                                                  "MR1",
...
chosenEstimator <- "WLSMV" # weighted least squares means and variance adjusted, for ordinal data (as recommended by lavaan)
...
efa_d1_beetle_unrotated <- fa(d1_beetle, 39, rotate = "none",
...
# run EFA without rotation with N factors
efa_d2_robot_unrotated <- fa(d2_robot, 39, rotate = "none",
                             cor = chosenCorType, fm = "minres")
...
# run EFA without rotation with N factors
efa_d2_beetle_unrotated <- fa(d2_beetle, 39, rotate = "none",
                              cor = chosenCorType, fm = "minres")
...
ggplot(aes(x = F1_mean, y = F2_mean, label = condition, colour = condition), 
  ...
  labs(title = "Study 4: Average factor scores (F1 vs. F2)\n",
       x = "\nAverage score: F1",
       y = "Average score: F2\n")
```

```{r s4 efa plot F1 vs F3, echo = F, fig.width = 8, fig.height = 8}
ggplot(aes(x = F1_mean, y = F3_mean, label = condition, colour = condition), 
       data = efa_d4_all_rotatedN_scores) +
  ...
  # scale_x_continuous(limits = c(-3.6, 3.6), breaks = seq(-3, 3, 0.5)) +
  # scale_y_continuous(limits = c(-3.6, 3.6), breaks = seq(-3, 3, 0.5)) +
  labs(title = "Study 4: Average factor scores (F1 vs. F3)\n",
       x = "\nAverage score: F1",
       y = "Average score: F3\n")
```

```{r s4 efa plot F2 vs F3, echo = F, fig.width = 8, fig.height = 8}
ggplot(aes(x = F2_mean, y = F3_mean, label = condition, colour = condition), 
       data = efa_d4_all_rotatedN_scores) +
  geom_errorbar(aes(ymin = F3_ci_lower, ymax = F3_ci_upper), 
                width = 0.01, alpha = 0.3) +
  ...
  # scale_x_continuous(limits = c(-3.6, 3.6), breaks = seq(-3, 3, 0.5)) +
  # scale_y_continuous(limits = c(-3.6, 3.6), breaks = seq(-3, 3, 0.5)) +
  labs(title = "Study 4: Average factor scores (F2 vs. F3)\n",
       x = "\nAverage score: F2",
       y = "Average score: F3\n")
```
